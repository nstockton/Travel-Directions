...
from travel.config import DATA_DIRECTORY, Config, ConfigError


class TestConfig(TestCase):
	@patch("travel.config.os")
	def test_load(self, mockOs: Mock) -> None:
		...
		with patch("travel.config.open", mock_open(read_data="invalid")):
			with self.assertRaises(ConfigError):
				cfg.reload()
		with patch("travel.config.open", mock_open(read_data="{}")):
			cfg.reload()
		cfg["test"] = "somevalue"
	...
	def test_save(self) -> None:
		...
		with patch("travel.config.open", mockOpen):
			cfg.save()
